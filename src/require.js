...
(function (global) {
    ...
    function isLink ( url ) {
        return /\.(css|less|scss|sass)$/i.test( url );
    }
    ...
    var createScripts = function (queues, onerror) {
        ...
        for (var i = 0, len = queues.length; i < len; i++) {
            ...
            if ( isLink(url) ) {
                ...
                //https不需要引用此H5跨域资源特性 by okay+ curt 19/4/9
                // script.setAttribute('crossorigin','anonymous');
                script.src = url;
            }
            if(/(msie\s|trident.*rv:)([\w.]+)/i.test(navigator.userAgent) && isLink(url)){
